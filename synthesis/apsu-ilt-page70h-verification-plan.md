...
Formal pass/fail for this plan is based on CMIS Page 70h behavior plus external lab measurements. Vendor CDB, lab-only P253.66 debug CDB, and Sian3 cAPI are diagnostic and correlation paths only. BERT APIs, traffic/FEC counters, loss, temperature, and timing captures are external measurement evidence.
...
| Diagnostic interfaces | Vendor CDB, lab-only P253.66 debug CDB, Sian3 cAPI, and module logs. |
...
| Reference baseline | IEEE P802.3dj D2.3, OIF2026.093.08 Page 70h, Broadcom Sian3 APSU AN, current customer-note plan, and current verification report. |
...
|---|---|---|---|
| Gate 0 | Baseline freeze and draft/source diff | Prevent stale assumptions. | Matrix frozen; draft re-diff complete; changes recorded. |
...
| Test ID | Gate | Objective | Baseline source | Interface under test | Supplemental evidence / correlation paths | Required evidence | Gap mitigated |
|---|---|---|---|---|---|---|---|
